...
document.querySelector('#save').addEventListener('click', () => {
  ...
  let link = document.querySelector('#popup a');
  ...
  xmlhttp.onloadend = function(e) {
    ...
    let responseId = JSON.parse(xmlhttp.responseText);
    popup.style.display = "flex";
    shadow.style.display = "block";
    // link it along with the id
    link.href = "/display.html?id=" + responseId.id;
    link.textContent = window.location.protocol + "//" + window.location.host + "/display.html?id=" + responseId.id;
  }
...
document.querySelector('#imgUpload').addEventListener('change', () => {
  ...
  xhr.onloadend = function(e) {
    ...
    newImage.src = "http://ecs162.org:3000/images/" + xhr.responseText; //selectedFile.name;
